...
export function startCronMemoryCleanup(memory: Memory): void {
  const sweep = async () => {
    try {
      ...
      if (deleted > 0) console.log(`[skillit] memory cleanup: deleted ${deleted} stale cron thread(s)`);
    } catch (err) {
      console.error('[skillit] memory cleanup failed:', err);
    }
  };

  // Catch up at boot in case the process was down for longer than the TTL,
